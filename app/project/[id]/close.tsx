...
export default function ProjectSettingsActions() {
  ...
  async function fetchProject() {
    if(!id) return;
    const { data } = await supabase.from("tournages").select("*").eq("id", id).single();
  ...
  async function handleCloseProject() {
      if(!id) return;
      if(!videoUrl) {
          Alert.alert("Lien manquant", "Veuillez ajouter un lien ou uploader une vidéo.");
          return;
      }
      ...
      try {
          ...
          Alert.alert("Félicitations 🎬", "Votre projet est maintenant dans le Hall of Fame !");
  ...
  if (project.status === 'completed') {
      return (
          <View style={styles.container}>
              ...
              <View style={[GlobalStyles.card, { alignItems: 'center', marginTop: 50 }]}>
                  <Text style={{ fontSize: 50 }}>🏆</Text>
                  <Text style={[GlobalStyles.title2, { textAlign: 'center', marginTop: 10 }]}>Ce projet est terminé !</Text>
                  <Text style={{ textAlign: 'center', color: '#666', marginTop: 5, marginBottom: 20 }}>
                      Il est visible dans le Hall of Fame.
                  </Text>
                  <TouchableOpacity onPress={() => router.push("/hall-of-fame")} style={GlobalStyles.primaryButton}>
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        <View style={styles.warningCard}>
            <Ionicons name="warning" size={32} color={Colors.light.danger} style={{ marginBottom: 10 }}/>
            <Text style={styles.warningTitle}>Zone de Danger</Text>
            <Text style={styles.warningText}>
                Clôturer le projet marquera la fin du tournage et le rendra public dans le Hall of Fame.
                Vous ne pourrez plus publier d'annonces de casting pour ce projet.
            </Text>
            ...
            >
                <Text style={styles.dangerButtonText}>Terminer et Clôturer le Projet</Text>
            </TouchableOpacity>
        </View>
          ...
          <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                  ...
                  <Text style={{ marginBottom: 15, color: '#444' }}>
                      Ajoutez le lien vers le résultat final ou uploadez votre bande annonce (max 30 Mo).
                  </Text>
                  
                  <View style={{ gap: 10, marginBottom: 20 }}>
                      <Text style={{ fontWeight: '600', color: '#333' }}>Option 1: Lien Externe</Text>
                      <TextInput
                      ...
                      <Text style={{ fontWeight: '600', color: '#333', marginTop: 10 }}>Option 2: Uploader une vidéo</Text>
                      <TouchableOpacity 
                        style={[GlobalStyles.secondaryButton, { borderColor: Colors.light.primary }]}
                        onPress={pickVideo}
                        disabled={uploading}
                      >
                         {uploading ? (
                             ...
                             <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8}}>
                                 ...
                                 <Text style={{ color: Colors.light.primary, fontWeight: '600' }}>Choisir un fichier</Text>
                             </View>
                         )}
                      </TouchableOpacity>
                      {uploading && <ClapLoading size={20} color={Colors.light.primary} />}
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
                      ...
                      <TouchableOpacity 
                        style={[GlobalStyles.primaryButton, { flex: 1, backgroundColor: Colors.light.success }]}
                        onPress={handleCloseProject}
                        disabled={closing || uploading || !videoUrl}
                      >
                          {closing ? <ClapLoading color="white"/> : <Text style={GlobalStyles.buttonText}>Publier ! 🍿</Text>}
                      </TouchableOpacity>
                  </View>
              </View>
          </View>
