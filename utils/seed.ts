...
const MOCK_USERS_DATA = [
  { full_name: "Alice Martin", role: "acteur", city: "Paris", bio: "Comédienne passionnée, 5 ans d'expérience théâtre." },
  { full_name: "Bob Dupont", role: "son", city: "Lyon", bio: "Ingénieur son freelance, équipé Zoom F8." },
  { full_name: "Charlie Vane", role: "image", city: "Marseille", bio: "Chef Opérateur, amoureux du 35mm." },
  { full_name: "David Chen", role: "realisateur", city: "Paris", bio: "Réalisateur de clips et courts-métrages." },
  { full_name: "Eva Green", role: "hmc", city: "Bordeaux", bio: "Maquilleuse FX pro." },
  { full_name: "Frank Castle", role: "technicien", city: "Lille", bio: "Electro / Machino, dispo toute la France." },
];
  ...
  try {
    ...
    for (let i = 0; i < MOCK_USERS_DATA.length; i++) {
      ...
      if (!error) {
        ...
        console.log(`Could not create fake profile (likely auth FK constraint): ${error.message}`);
    ...
    if (projectIds.length < 3) {
       ...
       const newProject = await supabase.from("tournages").insert({
    ...
    for (const pid of projectIds) {
        // Ensure roles exist
        const { data: roles } = await supabase.from("project_roles").select("id, category").eq("tournage_id", pid);
        
        let roleIds = roles?.map(r => r.id) || [];
        
        // If no roles, create some
        if (roleIds.length === 0) {
            const r1 = await supabase.from("project_roles").insert({
                tournage_id: pid,
                title: "Rôle Test A",
                category: "acteur",
                status: "open"
            ...
            const r2 = await supabase.from("project_roles").insert({
                tournage_id: pid,
                title: "Rôle Test B",
                category: "son",
                status: "open"
        ...
        const now = new Date();
        // Event 1: Tomorrow
        const d1 = new Date(now); d1.setDate(d1.getDate() + 1); d1.setHours(10, 0, 0);
        // Event 2: In 3 days
        const d2 = new Date(now); d2.setDate(d2.getDate() + 3); d2.setHours(14, 30, 0);
    ...
    Alert.alert("Succès", `Données générées !\n- ${createdFakeUserIds.length} Profils Fake\n- Connexions & Candidatures\n- Événements & Messages`);

  } catch (e) {
    console.error(e);
    Alert.alert("Erreur", "Une erreur est survenue (voir console). Possible contrainte Auth.");
  }
}
